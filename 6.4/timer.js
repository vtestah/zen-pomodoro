...
class TimerQueue {
    ...
    // Restore the queue to a given position without starting it (used for
    // paused session recovery). Keeps the queue stopped and prevents the
    // queue-started signal from firing again on the next start().
    setPosition(pos) {
        if (typeof pos !== 'number' || pos < 0 || pos >= this._queue.length) {
            return false;
        }
        this._queuePos = pos;
        this._isFirstStart = false;
...
class Timer {
    ...
    // Restore a paused timer at a specific remaining tick count without running
    // it. The next start() will resume from this value (paused semantics).
    setRemaining(ticks) {
        if (typeof ticks !== 'number' || isNaN(ticks) || ticks < 0) {
            return this;
        }
        this._clearTickTimeout();
        this._currentTickCount = ticks;
        this._isFirstStart = true;
        return this;
    }
